...
insert into Departamentos values(4,'Análise de Sistemas');
...
insert into Funcionarios values(12,'Caleb Vinicius','Enzo Rodrigues','1997-01-29','857.049.084-46','27.742.865-8','Rua Seis','51150-234','Recife','(81) 2524-5619',4,'Telefonista','3.630');
...
-- (Inner Join) Liste o nome completo do funcionário, o nome do departamento e a função de todos os funcionários
select Funcionarios.Codigo_func,Funcionarios.NomeFunc,Funcionarios.Sobrenome,Departamentos.NomeDepto,Funcionarios.Funcao from Departamentos inner join Funcionarios on Departamentos.Codigo_depto=Funcionarios.CodigoDepartamento;

-- (Inner Join) Liste os nomes dos departamentos e dos funcionários que têm a função de supervisor
select Departamentos.NomeDepto,Funcionarios.NomeFunc,Funcionarios.Funcao from Departamentos inner join Funcionarios on Departamentos.Codigo_depto=Funcionarios.CodigoDepartamento where Funcionarios.Funcao = 'supervisor';

-- (Inner Join) Liste o nome do departamento e nome completo do funcionário ordenados por departamento e Sobrenome do funcionário
select Departamentos.NomeDepto,Funcionarios.NomeFunc,Funcionarios.Sobrenome from Departamentos inner join Funcionarios on Departamentos.Codigo_depto=Funcionarios.CodigoDepartamento order by Departamentos.NomeDepto,Funcionarios.Sobrenome;

-- (Group by) Liste o valor da folha de pagamento de cada departamento (nome)
select Departamentos.NomeDepto, sum(Funcionarios.Salario) from Departamentos inner join Funcionarios on Departamentos.Codigo_depto = Funcionarios.CodigoDepartamento  group by Departamentos.NomeDepto;

-- (Group by) Liste o menor salário pago pela empresa em cada departamento
select Departamentos.NomeDepto, min(Funcionarios.Salario) from Departamentos inner join Funcionarios on Departamentos.Codigo_depto = Funcionarios.CodigoDepartamento group by Departamentos.NomeDepto;

-- (SubQuery) Liste os nomes dos funcionários que trabalham no departamento Pessoa
select NomeFunc, Sobrenome from Funcionarios where CodigoDepartamento = (select Codigo_depto from Departamentos where NomeDepto = 'Financeiro'); 
