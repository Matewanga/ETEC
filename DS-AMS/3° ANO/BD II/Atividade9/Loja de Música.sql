...
select Cds.Nome_cd, convert(time, dateadd(second, sum(datediff(second, '00:00:00', Musicas.Tempo)), '00:00:00')), count(*) from Cds join Musicas on Cds.codigo_cd = Musicas.codigo_cd group by Cds.Nome_cd;

--  (Group by) Mostre a quantidade de Cds
select count(*) from Cds;

-- (Group by) Mostre a quantidade de músicas por cds
select Cds.codigo_cd, Cds.Nome_cd, count(Musicas.codigo_cd) from Cds inner join Musicas on Cds.codigo_cd = Musicas.codigo_cd group by Cds.codigo_cd, Cds.Nome_cd;
