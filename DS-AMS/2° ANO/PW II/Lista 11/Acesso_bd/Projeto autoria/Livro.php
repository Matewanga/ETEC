...
class Livro
{
    ...
    function alterar2()
    {
        ...
        {
            $this->conn = new Conectar();
            $sql = $this->conn->prepare("UPDATE livro SET titulo = ?, categoria = ?,ibsn = ?,idioma = ?,qtdepag = ? WHERE cod_livro = ?");
            @$sql-> bindParam(1, $this->getTitulo(), PDO::PARAM_STR);
            @$sql-> bindParam(2, $this->getCategoria(), PDO::PARAM_STR);
            @$sql-> bindParam(3, $this->getIbsn(), PDO::PARAM_STR);
            @$sql-> bindParam(4, $this->getIdioma(), PDO::PARAM_STR);
            @$sql-> bindParam(5, $this->getQtdepag(), PDO::PARAM_STR);
            @$sql-> bindParam(6, $this->getCod_livro
            (), PDO::PARAM_STR);
            if($sql->execute() == 1)
            {
